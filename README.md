...
- **Description**: Something a bit more detailed than 'Category', but could still come up again.
  Like specifying which brand of gas station you went to. There's a fuzzy search here to help keep
  things concise, don't worry! It's also optional by default, so make sure you enable "require
...
If you don't want to mess with Docker or command lines, you can just download the pre-built
application.

1. Go to the [Releases page](../../releases) and download the `.zip` for your operating system
   (Windows, macOS, or Linux).
2. Extract the **entire** zip folder (it contains the executable, database migrations, and necessary
   production files).
3. Run the `jaft` executable inside the extracted folder.
4. Open your browser and go to the address it provides!

Feel free to make a shortcut and store it wherever you want, but the application files (and your
finance.db) are going to have to make their home in that unzipped folder.

### Docker (Recommended for Servers)

JAFT is also available as a multi-platform Docker image (`linux/amd64` and `linux/arm64`), natively built
to run fast on pretty much anything.

Two image tags are available:

- `latest` - stable releases, what you probably want
- `dev` - built automatically on every merge to the dev branch, so it's cutting-edge but might have
  bugs and such
...
- [SQLite](https://sqlite.org) - local database via [bun:sqlite](https://bun.sh/docs/api/sqlite)
